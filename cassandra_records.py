...
class RecordCollection(object):
    ...
    def __repr__(self):
        r = '<RecordCollection size={} pending={}>'.format(
            len(self), self.pending)
        return r
...
class Database(object):
    ...
    def __init__(self,
                 hosts=None, port=9042, keyspace=None,
                 username=None, password=None):
        # If no hosts was provided, fallback to localhsot.
        if hosts is not None and isinstance(hosts, basestring):
            hosts = hosts.split(",")
        ...
        if username is not None and password is not None:
            ...
            cluster_params["auth_provider"] = PlainTextAuthProvider(
                username=username, password=password)

        self.cluster = Cluster(
            contact_points=hosts, port=port, **cluster_params)
    ...
    def get_table_names(self, keyspace=None):
        """Returns a list of table names for the specified keyspace."""

        metadata = self.session.cluster.metadata

        # Serve the table names.
        keyspace = keyspace or self.keyspace
        return metadata.keyspaces[keyspace].tables.keys()
    ...
    def query(self, query, fetchall=False, **params):
        ...
        cursor = self.session.execute(query, **params)  # TODO: PARAMS GO HERE

        # Row-by-row Record generator.
        row_gen = (Record(row._fields, row) for row in cursor)
...
def cli():
    cli_docs = """cassandra-records: CQL for aliens™
A (blatant copy of a Kenneth Reitz) project.

Usage:
  cassandra-records <query> <format> [<params>...] [--hosts=<hosts>] [--port=<port>] [--keyspace=<keyspace>] [--username=<username>] [--password=<password>]
  cassandra-records (-h | --help)

Options:
  -h --help     Show this screen.
  --host=<host>  The Cassandra host(s) to use.
  --port=<port>  The Cassandra port to use.
  --keyspace=<keyspace>  The Cassandra keyspace to use.
  --username=<username>  The username to use for login.
...
Supported Formats:
   ...
   Note: xls, xlsx, dbf, and ods formats are binary, and should only be
         used with redirected output e.g. '$ cassandra-records sql xls > sql.xls'.
...
Notes:
  - Query is intended to be the path of a CQL file, however a query string
  ...
  - Records is intended for report-style exports of database queries, and
    ...
    # Create the Database.
    db = Database(
        hosts=arguments['--hosts'], port=arguments['--port'],
